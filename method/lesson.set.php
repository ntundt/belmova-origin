...
if (isset($parameters['uid']) and $continue) {
	...
	if ($user->hasRightTo('createLessons')) {
		makeResponse(LessonsList::setLesson($parameters['partition_id'], $parameters['topic_id'], $parameters['topic_level'], $parameters['lesson_number'], $parameters['json_object']), ErrorList::makeAssoc());
	} else {
